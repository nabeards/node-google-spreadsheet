...
class GoogleSpreadsheetWorksheet {
  ...
  async loadHeaderRow(headerRow = 1) {
    const rows = await this.getCellsInRange(`A${headerRow}:${this.lastColumnLetter}${headerRow}`);
    if (!rows) {
  ...
  async setHeaderRow(headerValues, headerRow = 1) {
    ...
    const response = await this._spreadsheet.axios.request({
      method: 'put',
      url: `/values/${this.encodedA1SheetName}!${headerRow}:1`,
      params: {
        valueInputOption: 'USER_ENTERED', // other option is RAW
        includeValuesInResponse: true,
      },
      data: {
        range: `${this.a1SheetName}!${headerRow}:1`,
        majorDimension: 'ROWS',
        values: [[
          ...trimmedHeaderValues,
